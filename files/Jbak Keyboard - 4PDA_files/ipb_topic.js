...
function ins(name,id){my_show_div(my_getbyid('qr_open'));if(document.REPLIER.post){var input=document.REPLIER.post;input.value=input.value+(id?'[SNAPBACK]'+id+'[/SNAPBACK] ':'')+"[b]"+name+",[/b]\n"}}
		...
		,	do_request_function = function()
			{
				...
				r2 = [ qId ( "pb-"+pid+"-r2" ), qId ( "pb-"+pid+"-r2", tmp ) ];
				r3 = [ qId ( "pb-"+pid+"-r3" ), qId ( "pb-"+pid+"-r3", tmp ) ];
				if ( !r2[0] || !r2[1] || !r3[0] || !r3[1] )
				{
					document.body.removeChild ( ifrm );
					return;
				}
				r2[0].innerHTML = r2[1].innerHTML;
				r3[0].innerHTML = r3[1].innerHTML;
				tmp = qEls ( "script", r3[0] );
